...
func tradingPairStatements(db *sqlx.DB) (*tradingPairStmts, error) {
	...
	const updateTradingPairQuery = `UPDATE "trading_pairs"
									SET price_precision  = coalesce(:price_precision, price_precision),
									    ...
									    stale_threshold = coalesce(:stale_threshold, stale_threshold)
	...
	tp.price_limit_min, tp.price_limit_max, tp.min_notional,tp.stale_threshold	FROM trading_pairs tp WHERE tp.id=$1
	...
	tpd.price_limit_min, tpd.price_limit_max, tpd.min_notional,tpd.stale_threshold FROM trading_pairs_deleted tpd WHERE tpd.id=$1 and $2 IS TRUE
...
func assetStatements(db *sqlx.DB) (*sqlx.NamedStmt, *sqlx.Stmt, *sqlx.NamedStmt, *sqlx.Stmt, error) {
	...
	const updateAssetQuery = `WITH updated AS (
		...
		SET symbol       = COALESCE(:symbol, symbol),
		    ...
		    rebalance_price_offset = COALESCE(:rebalance_price_offset,rebalance_price_offset),
