...
func (self *BoltStorage) GetActivity(id common.ActivityID) (common.ActivityRecord, error) {
	...
	err := self.db.View(func(tx *bolt.Tx) error {
		...
		v := b.Get(idBytes[:])
		if v == nil {
			return fmt.Errorf("Cannot find that activity")
		}
...
func (self *BoltStorage) GetPendingTargetQty() (metric.TokenTargetQty, error) {
	...
	err = self.db.View(func(tx *bolt.Tx) error {
		...
		if data == nil {
			err = fmt.Errorf("There no pending target quantity")
		} else {
...
func (self *BoltStorage) StorePendingTargetQty(data, dataType string) error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		if lastPending != nil {
			err = fmt.Errorf("There is another pending target quantity. Please confirm or cancel it before setting new target.")
			return err
		}
...
func (self *BoltStorage) RemovePendingTargetQty() error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		if lastPending == nil {
			return fmt.Errorf("There is no pending target quantity.")
		}
...
func (self *BoltStorage) StoreTokenTargetQty(id, data string) error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		if pendingTargetQty == nil {
			err = fmt.Errorf("There is no pending target activity to confirm.")
			return err
		} else {
			// verify confirm data
			...
			if tokenTargetQty.ID != idInt {
				err = fmt.Errorf("Pending target quantity ID does not match")
				return err
			}
			if data != pendingData {
				err = fmt.Errorf("Pending target quantity data does not match")
				return err
			}
...
func (self *BoltStorage) StorePendingPWIEquation(data string) error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		_, v := c.First()
		if v != nil {
			err = fmt.Errorf("There is another pending equation, please confirm or reject to set new equation")
			return err
		} else {
			idByte := uint64ToBytes(timepoint)
...
func (self *BoltStorage) GetPendingPWIEquation() (metric.PWIEquation, error) {
	...
	err = self.db.View(func(tx *bolt.Tx) error {
		...
		_, v := c.First()
		if v == nil {
			return fmt.Errorf("There no pending equation")
		} else {
...
func (self *BoltStorage) StorePWIEquation(data string) error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		_, v := c.First()
		if v == nil {
			err = fmt.Errorf("There no pending equation")
			return err
		} else {
			...
			if pending.Data != data {
				err = fmt.Errorf("Confirm data does not match pending data")
				return err
			}
...
func (self *BoltStorage) GetPWIEquation() (metric.PWIEquation, error) {
	...
	err = self.db.View(func(tx *bolt.Tx) error {
		...
		_, v := c.Last()
		if v == nil {
			err = fmt.Errorf("There is no equation")
			return err
		} else {
			json.Unmarshal(v, &result)
		}
...
func (self *BoltStorage) RemovePendingPWIEquation() error {
	...
	err = self.db.Update(func(tx *bolt.Tx) error {
		...
		if k != nil {
			...
			err = fmt.Errorf("There is no pending data")
		}
		return err
	})
	return err
...
func (self *BoltStorage) GetExchangeStatus() (common.ExchangesStatus, error) {
	...
	err = self.db.View(func(tx *bolt.Tx) error {
		...
		_, v := c.Last()
		if v == nil {
			err = fmt.Errorf("There is no data yet.")
			return err
		}
